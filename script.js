...
document.addEventListener('DOMContentLoaded',()=>{const imgs=document.querySelectorAll('.gallery-grid img');const lb=document.getElementById('galleryLightbox');const lbi=document.getElementById('galleryLightboxImg');if(!lb)return;imgs.forEach(img=>img.addEventListener('click',()=>{lb.classList.add('active');lbi.src=img.src;}));lb.addEventListener('click',()=>lb.classList.remove('active'));});
