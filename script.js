...
/* MOBILE BUTTONS — HAKYHM
   Hamburger = navigation. Share = social drawer. */
document.addEventListener('DOMContentLoaded', () => {
  ...
  const closeSocial = (restoreFocus = false) => {
  ...
  menuButton?.addEventListener('click', () => {
    navLinks?.classList.contains('mobile-open') ? closeNavigation(true) : openNavigation();
  });

  navLinks?.querySelectorAll('a').forEach(link => link.addEventListener('click', () => closeNavigation(false)));

  socialButton?.addEventListener('click', (event) => {
    event.preventDefault();
    socialDrawer?.classList.contains('is-open') ? closeSocial(true) : openSocial();
  });

  socialClose?.addEventListener('click', () => closeSocial(true));
  socialDrawer?.addEventListener('click', event => { if (event.target === socialDrawer) closeSocial(true); });
  socialDrawer?.querySelectorAll('.social-drawer-links a').forEach(link => link.addEventListener('click', () => closeSocial(false)));

  document.addEventListener('keydown', event => {
    if (event.key !== 'Escape') return;
    if (socialDrawer?.classList.contains('is-open')) closeSocial(true);
    else if (navLinks?.classList.contains('mobile-open')) closeNavigation(true);
  });

  window.addEventListener('resize', () => {
    if (window.innerWidth > 900) { closeNavigation(false); closeSocial(false); }
  });
});
